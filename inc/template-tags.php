...
if ( ! function_exists( 'flexieduca_posted_on' ) ) :
	...
	function flexieduca_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'flexieduca' ),
			'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
		);

		$byline = sprintf(
			/* translators: %s: post author. */
			esc_html_x( 'by %s', 'post author', 'flexieduca' ),
	...
	function flexieduca_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'flexieduca' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'flexieduca' ),
						array(
							'span' => array(
								'class' => array(),
							),
